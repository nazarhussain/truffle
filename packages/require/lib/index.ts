...
import { compile, registerTsNode } from "./compiler";
...
export function file(options: RequireOptions) {
  ...
  const sandbox: { [index: string]: any } = {
    __filename: path.basename(scriptPath),
    ...
    require: (id: string) => {
      ...
      if (id[0] === ".") {
        return originalRequire(path.join(path.dirname(scriptPath), id));
      } else {
  ...
  registerTsNode(conf, scriptPath, context);

  const source = compile(conf, scriptPath);
